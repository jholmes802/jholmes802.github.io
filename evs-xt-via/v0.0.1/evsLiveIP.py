...
def loadIPJson(serverIP: str, liveIPSourceFile: str) -> tuple[str, dict]:
    """Provided the Server Number and the location of the work sheet file, this can create a JSON file containing the needed JSON information about the server and its LiveIP information. This will also return the path to the JSON file that was created.

    Args:
        serverNumber (str): '02' This is the server number as it shows up in the sheets. IF the sheets are as '2' then you need to pass that. This is a known limitation of the code.
        liveIPSourceFile (str): Path to the CSV file that is the export of the worksheets Multicast IP A-UNIT.
    ...
    rtr_nameName = headers[rtr_nameIndex]
    port_or_connectorIndex = headers.index("PORT_OR_CONNECTOR")
    port_or_connectorName = headers[port_or_connectorIndex]
    directionIndex = headers.index("DIRECTION")
    directionName = headers[directionIndex]
    udpPortIndex = headers.index("UDP_PORT")
    udpPortName = headers[udpPortIndex]
    videoIndex = headers.index("VIDEO_PURPLE")
    videoName = headers[videoIndex]
    audioG1Index = headers.index("AUDIO_G1_PURPLE")
    audioG1Name = headers[audioG1Index]
    audioG2Index = headers.index("AUDIO_G2_PURPLE")
    audioG2Name = headers[audioG2Index]
    ancIndex = headers.index("ANC_PURPLE")
    ancName = headers[ancIndex]

    if DEBUG:
        print(headers)

    result = {}
    for i, row in enumerate(dataRead[2:]):
        # print(row)
        if row[0] == deviceName.upper():
            ...
            for j, ipRow in enumerate(dataRead[startIndex - 1 : endIndex]):
                # This checks if there is not enough data in the row. Some rows do not include multicast information.
                if len(ipRow) < 18:
                    continue
                # If the row is not captured in result.keys(), we will build it as equal to a blank dictionary.
    ...
    outPutFilePath = f"./tmp/MulticastIP{serverNumber}.json"
    json.dump(result, open(outPutFilePath, "w"))
...
def buildLiveIPcsv(
    ...
    """This builds the CSV file that is used to configure the LiveIP. This is what you could upload to the server.
    ...
    Args:
        ...
        workSheetData (dict): Data from the worksheets, the IP information. This is the JSON file that was created.

    Returns:
        str: Output file path if successful.
    """
    logger.info(f"Building LiveIP CSV for Server Number {serverNumber}")
    serverNumber = "{:02}".format(serverNumber)
    ...
    resultHeaderData = [version, "59.94", numIn, numOut, slsm1, slsm2, slsm1spd, slsm2spd, "X", "", "", ""]
    ...
    def buildVideoIn():
        ...
        for x in range(1, numIter):
            if slsm1 == "0" or (i > int(slsm1)):
                if slsm1 != "0":
                    x = i
                tempLine = [f"IN{x} Video", "PRI", "X", "", "192.168.0.1", "", 30000, "239.1.1.1", 30000, "C"]
            else:
                j = x % int(slsm1spd.replace("x", ""))
                if j == 0:
                    j = int(slsm1spd.replace("x", ""))
                # print(f"i: {i}, j: {j}")
                tempLine = [f"IN{i} PHASE-{j}", "PRI", "X", "", "192.168.0.1", "", 30000, "239.1.1.1", 30000, "C"]
    ...
    def buildAncIn():
        ...
        array.append(header)
        for x in range(1, numIn + 1):
            tempLine = [f"IN{x} Anc-1", "PRI", "X", "", "192.168.0.1", "", 30000, "239.1.1.1", 30000, "C"]
            array.append(tempLine)
    ...
    def buildMVVideoIn():
        ...
        array.append(header)
        for x in range(1, 3):
            tempLine = [f"MV IN{x} Video", "PRI", "X", "", "192.168.0.1", "", 30000, "239.1.1.1", 30000, "C"]
            array.append(tempLine)
    ...
    finalWriter = csv.writer(open(output, "w"), quoting=csv.QUOTE_MINIMAL)
    finalWriter.writerows(finalResult)
...
def getconfigLineData(serverIP: str, version: str, worksheetData: dict, configLineNumber: int = None):
    ...
    for i in tqdm.tqdm(numIter):
        reqData = requests.get(
            f"http://{serverIP}/cfgweb/CfgWeb.dll/GetConfigValuesJS?SessionID=Reset&NumLine={i}&Meta=true&Reset=false&All=true"
        )
...
def sendLiveIPs(serverIP: str, configLineNumber: int = None):
    ...
    def getSessionID() -> requests.Response:
        ...
        r = requests.get(f"http://{serverIP}/ipconfigurationweb/setlinenumberactionjs?SessionId=Reset&number={i}")
    ...
    def setLineNumber(lineNum: int = 1, sessionID: str = "Reset") -> requests.Response:
        ...
        r = requests.get(f"http://{serverIP}/ipconfigurationweb/setlinenumberactionjs?SessionId={sessionID}&number={i}")
        return r.json()
    ...
    def commit(sessionID: str) -> requests.Response:
        ...
        r = requests.get(f"http://{ serverIP }/ipconfigurationweb/commitactionjs?SessionId={sessionID}")
        return r
    ...
    def needToBeUpdated(sessionID: str = "Reset") -> requests.Response:
        ...
        r = requests.get(f"http://{serverIP}/ipconfigurationweb/needtobeupdatedactionjs?SessionId={sessionID}")
        return r

    # This handles setting the numIter which is what is itterated over, so if it is None, we set it to 1-16, otherwise we set it to an list.
    if configLineNumber is None:
        numIter = range(1, 17)
    elif isinstance(configLineNumber, int):
        numIter = [configLineNumber]
    ...
    """
    This is the master Loop, this will set send all the LiveIPs based on the iterable that is passed into tqdm.
...
def main():
    ...
    parser.add_argument("--operation", type=str, help="Operation, options are [build, send, buildSend]", required=False)
