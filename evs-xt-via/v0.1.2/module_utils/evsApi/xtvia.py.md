...
DEFAULT_TIMEDELTA = datetime.timedelta(minutes=10)
...
class xtviaServer:
    def __init__(
        self,
        serverIP: str,
        configsToCollect: list = None,
        inspection: int = 10,
        sessionID: str = "Reset",
        singleParam: str = None,
        getLiveIPs: bool = False,
    ) -> None:
        ...
        self._lastUpdateTS: datetime.datetime = datetime.datetime.now()
        if configsToCollect is None:
            self._foucsConfigLines = range(1,17)
        else:
            self._foucsConfigLines: list = configsToCollect
        self._currentLiveIPFocus: int = None
        ...
        if len(self._foucsConfigLines) == 0:
            self._foucsConfigLines = [self._get_multicam_info()["SharedNumLineLoaded"]]
        if inspection >= 2:
    ...
    def _post_request(self, url: str, params: dict, data: dict = {}) -> dict:
        ...
        Returns:
            dict: Returns the JSON data from the request.
        """
        logger.debug("Sending post request with params: {params} and data: {data}".format(params=params, data=data))
        
        response = requests.post(url, params=params, data=data)
        
        logger.debug("Got response: {response}".format(response=response.json()))
    ...
    def _set_audio_type(self, numLine: int, audioType: str, audioDirection: str, save: bool = True) -> dict:
        ...
        url = f"http://{self._serverIP}/cfgweb/CfgWeb.dll/SetAudioTypeJS"

        data = self._post_request(url, params)
        self._save(numLine=numLine, save=save)
        return data

    def _set_disk_use(self, numLine: int, channel_in_count: int, save: bool = True) -> dict:
        singleChannel = 100 / channel_in_count
        singleChannel = math.floor(singleChannel)
        remainder = 100 - (singleChannel * channel_in_count)
        params: dict = {
            "NumLine": numLine,
            "SessionID": self._get_SessionID(),
            "Commit": save,
        ...
        for i in range(1, channel_in_count + 1):
            params[f"CFG_PARAM_REC_DISKUSE_{i}"] = singleChannel
        params[f"CFG_PARAM_REC_DISKUSE_1"] = remainder + singleChannel
    ...
    def _save(self, numLine: int, save: bool = True) -> dict:
        ...
        if "Error" in data.keys():
            raise ValueError("Unable to save config {line_num} due to {msg}".format(line_num=numLine, msg=data["Error"]["Msg"]))
    ...
    def _get_liveip_one(self, numLine: int, uri: str) -> dict:
        ...
        response: dict = self._post_request(url, params, data)

        # Because we get a huge dictionary response we need to process it down a bit so we can return the appropriate json dict.
        response = response["Data"]["Parameters"][uri]
    ...
    def _commit_liveIP(self) -> dict:
        ...
        if "Data" not in data.keys():
            raise ValueError("Unable to commit config {msg}".format(msg=data))
        
        if data["Data"]["Commit"] != "Success":
            raise ValueError("Unable to commit config {msg} failed success".format(msg=data))
        return data

    def _valid_liveIP(self) -> dict:
        params = {"SessionId": self._get_SessionID(False)}
        url = f"http://{self._serverIP}/ipconfigurationweb/validateactionjs"
        data = self._get_request(url, params)
        return data
